...
export const api = {
  ...
  createWorkspace: (name: string) =>
    request<Workspace>("/api/v1/workspaces", { body: { name } }),
  listProjects: (wid: string) =>
    request<{ items: Project[] }>(`/api/v1/workspaces/${wid}/projects`),
  createProject: (wid: string, name: string, description = "") =>
    request<Project>(`/api/v1/workspaces/${wid}/projects`, {
      body: { name, description },
    }),

  listCanvases: (pid: string) =>
    request<{ items: CanvasMeta[] }>(`/api/v1/projects/${pid}/canvases`),
  createCanvas: (pid: string, name: string) =>
    request<{ canvas: CanvasMeta }>(`/api/v1/projects/${pid}/canvases`, {
      body: { name },
    }),
  ...
  appendOps: (
    ...
    request<AppendOpsResult>(`/api/v1/canvases/${cid}/ops`, {
      body: { baseVersion, ops },
      idempotencyKey,
    }),
  exportCanvas: (cid: string) =>
    request<unknown>(`/api/v1/canvases/${cid}/export`),
  importCanvas: (
    pid: string,
    payload: ImportPayload,
    idempotencyKey?: string,
  ) =>
    request<ImportResult>(`/api/v1/canvases/${pid}/import`, {
      body: payload,
      idempotencyKey,
    }),

  createRun: (cid: string, targetNodes: string[], idempotencyKey?: string) =>
    request<RunDTO>(`/api/v1/canvases/${cid}/runs`, {
      body: { targetNodes },
      idempotencyKey,
    }),
  ...
  cancelRun: (rid: string) =>
    request<{ ok: boolean }>(`/api/v1/runs/${rid}/cancel`, { method: "POST" }),
  replayRun: (rid: string) =>
    request<RunDTO>(`/api/v1/runs/${rid}/replay`, { method: "POST" }),

  generate: (wid: string, payload: GeneratePayload, idempotencyKey?: string) =>
    request<RunDTO>(`/api/v1/workspaces/${wid}/generate`, {
      body: payload,
      idempotencyKey,
    }),
  ...
  assetRawUrl: (aid: string, wid: string) =>
    `/api/v1/assets/${aid}/raw?workspaceId=${encodeURIComponent(wid)}`,
  ...
  listProviders: (wid: string) =>
    request<{ items: Provider[] }>(`/api/v1/workspaces/${wid}/providers`),
  ...
  createCredential: (wid: string, pid: string, payload: CredentialInput) =>
    request<Credential>(
      `/api/v1/workspaces/${wid}/providers/${pid}/credentials`,
      { body: payload },
    ),
  ...
  listPromptSources: (wid: string) =>
    request<{ items: PromptSource[] }>(
      `/api/v1/workspaces/${wid}/prompt-sources`,
    ),
  createPromptSource: (wid: string, payload: PromptSourceInput) =>
    request<PromptSource>(`/api/v1/workspaces/${wid}/prompt-sources`, {
      body: payload,
    }),
  syncPromptSource: (wid: string, sid: string) =>
    request<SyncResult>(
      `/api/v1/workspaces/${wid}/prompt-sources/${sid}/sync`,
      { method: "POST" },
    ),
  ...
  listPlugins: (wid: string) =>
    request<{ items: Plugin[] }>(`/api/v1/workspaces/${wid}/plugins`),
  ...
  enablePlugin: (wid: string, key: string, enabled: boolean) =>
    request<Plugin>(`/api/v1/workspaces/${wid}/plugins/${key}/enable`, {
      body: { enabled, trusted: true },
    }),
  pluginBundleUrl: (key: string, version: string) =>
    `/api/v1/plugins/${key}/${version}/bundle`,

  createAgentSession: (wid: string, canvasId: string, backend = "http") =>
    request<AgentSessionDTO>("/api/v1/agent/sessions", {
      body: { workspaceId: wid, canvasId, backend },
    }),
  createAgentTurn: (sid: string, input: string) =>
    request<AgentTurnDTO>(`/api/v1/agent/sessions/${sid}/turns`, {
      body: { input },
    }),
  approveAgentTurn: (sid: string, tid: string, approve: boolean) =>
    request<AgentTurnDTO>(
      `/api/v1/agent/sessions/${sid}/turns/${tid}/approve`,
      { body: { approve } },
    ),
  agentHistory: (sid: string) =>
