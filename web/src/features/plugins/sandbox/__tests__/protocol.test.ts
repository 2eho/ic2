...
describe('protocol', () => {
  it('拒绝非法消息（必须丢弃而不是尽力解析）', () => {
    ...
    // 未知方法必须拒绝：白名单语义
    expect(parsePluginMessage({ type: 'host:call', id: 'x', method: 'process.exec' })).toBeNull();
    // 缺字段
    expect(parsePluginMessage({ type: 'host:call', method: 'node.get' })).toBeNull();
  });

  it('接受合法消息', () => {
    expect(parsePluginMessage({ type: 'plugin:ready' })).toEqual({ type: 'plugin:ready' });
    const call = parsePluginMessage({ type: 'host:call', id: '1', method: 'node.get' });
    expect(call).toEqual({ type: 'host:call', id: '1', method: 'node.get', params: undefined });
  });

  it('resize 消息必须是有限数', () => {
    expect(parsePluginMessage({ type: 'plugin:resize', width: Number.NaN, height: 10 })).toBeNull();
    expect(parsePluginMessage({ type: 'plugin:resize', width: 100, height: 200 })).toEqual({
      type: 'plugin:resize', width: 100, height: 200,
    });
  });

  it('日志级别回落到 info', () => {
    const m = parsePluginMessage({ type: 'plugin:log', level: 'debug', message: 'x' });
  ...
  it('权限判定支持 ai.generate 的子能力', () => {
    expect(hasPermission(['ai.generate:image'], 'ai.generate', 'image')).toBe(true);
    expect(hasPermission(['ai.generate:image'], 'ai.generate', 'video')).toBe(false);
