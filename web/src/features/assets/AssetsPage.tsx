...
export function AssetsPage({ t }: { t: TFn }) {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200, margin: "0 auto" }}>
      ...
      >
        <h1 style={{ fontSize: 20, margin: 0, marginRight: 12 }}>
          {t("assets.title")}
        </h1>
        <input
          className="ic-input"
      ...
      >
        {items.map((a) => (
          <div key={a.id} className="ic-card" style={{ padding: 8 }}>
            {a.kind === "image" ? (
              <img
                src={api.assetThumbUrl(a.id, workspaceId, 320)}
                alt={a.name}
                style={{
                  width: "100%",
                  height: 120,
              ...
              <div className="ic-empty" style={{ padding: 20, fontSize: 12 }}>
                {a.kind}
              </div>
            )}
            <div
            ...
            <div className="ic-dim ic-mono" style={{ fontSize: 11 }}>
              {Math.round(a.size / 1024)}KB · {a.mime}
            </div>
            <div style={{ display: "flex", gap: 4, marginTop: 6 }}>
              <a
                ...
                href={api.assetRawUrl(a.id, workspaceId)}
                download
              >
                {t("common.download")}
              </a>
