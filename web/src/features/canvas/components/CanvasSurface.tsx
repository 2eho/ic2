...
export function CanvasSurface({ t, kernel, onCommit, onRun }: Props) {
  ...
  const [connection, setConnection] = useState<{ x1: number; y1: number; x2: number; y2: number } | null>(null);
  ...
  const modifiers = (e: React.PointerEvent | React.MouseEvent | React.KeyboardEvent): Modifiers => ({
    shift: e.shiftKey,
    ctrl: e.ctrlKey,
    meta: e.metaKey,
    alt: e.altKey,
  ...
  const onPointerDown = (e: React.PointerEvent) => {
    ...
    setMenu(null);
    const screen = { x: e.clientX - (hostRef.current?.getBoundingClientRect().left ?? 0), y: e.clientY - (hostRef.current?.getBoundingClientRect().top ?? 0) };
    ...
    // 手柄命中优先（仅单选时）
    let handle: ReturnType<typeof import('../kernel/geometry').hitResizeHandle> = null;
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    kernel.notePointer(screen);
    const intent = kernel.interaction.handle({ type: 'pointermove', point: screen, modifiers: modifiers(e) });
  ...
  const onPointerUp = (e: React.PointerEvent) => {
    ...
    const intent = kernel.interaction.handle({ type: 'pointerup', point: screen });
    if (state === 'marquee') {
      ...
      const hits = kernel.scene.hitRect(rectWorld, { skipGroups: true }).map((n) => n.id);
  ...
  const onWheel = (e: React.WheelEvent) => {
    ...
    e.preventDefault();
    kernel.handleIntent({ type: 'zoom', point: screen, delta: e.deltaY * 0.0025 });
  };
  ...
  return (
    <div
      ref={hostRef}
      data-canvas-host
      style={{ position: 'absolute', inset: 0, overflow: 'hidden', touchAction: 'none', cursor: kernel.interaction.current === 'panning' ? 'grabbing' : 'default' }}
      ...
      onDoubleClick={(e) => {
        ...
        const world = kernel.viewport.toWorld(screen);
        const hit = kernel.scene.hitTest(world, { skipGroups: true });
        kernel.handleIntent(kernel.interaction.handle({ type: 'dblclick', point: screen, hitNode: hit?.id }));
      }}
      tabIndex={0}
    >
      ...
      <div ref={wrapperRef} style={{ position: 'absolute', top: 0, left: 0, willChange: 'transform' }}>
        <EdgesLayer kernel={kernel} />
        {visible.map((n) => (
          <NodeShell key={n.id} t={t} kernel={kernel} nodeId={n.id} onCommit={onCommit} onRun={onRun} />
        ))}
      </div>
      ...
      <svg style={{ position: 'absolute', inset: 0, pointerEvents: 'none', width: '100%', height: '100%' }}>
        {selection.nodes.map((id) => {
          ...
          const r = kernel.viewport.rectToScreen(n.rect);
          return (
            <rect key={id} x={r.x - 2} y={r.y - 2} width={r.w + 4} height={r.h + 4}
              fill="none" stroke="var(--ic-accent)" strokeWidth={1.5} strokeDasharray="4 3" />
          );
        })}
        {marquee && (
          <rect x={marquee.x} y={marquee.y} width={marquee.w} height={marquee.h}
            fill="var(--ic-accent-soft)" stroke="var(--ic-accent)" strokeWidth={1} />
        )}
        {connection && (
          <line x1={connection.x1} y1={connection.y1} x2={connection.x2} y2={connection.y2}
            stroke="var(--ic-accent)" strokeWidth={2} strokeDasharray="5 4" />
        )}
      </svg>
      ...
      <div style={{ position: 'absolute', left: 12, bottom: 12, display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        {kernel.settings.readOnly && <span className="ic-badge ic-badge--warn">{t('canvas.readonly')}</span>}
      </div>
    </div>
  );
}

function hitHandle(screen: { x: number; y: number }, r: { x: number; y: number; w: number; h: number }) {
  const importHit = (globalThis as unknown as { __icHitHandle?: typeof import('../kernel/geometry').hitResizeHandle }).__icHitHandle;
  ...
  for (const [k, c] of Object.entries(centers)) {
    if (Math.abs(screen.x - c.x) <= hs / 2 && Math.abs(screen.y - c.y) <= hs / 2) {
