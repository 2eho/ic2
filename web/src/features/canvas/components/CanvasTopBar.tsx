...
export function CanvasTopBar({
  ...
  return (
    ...
    >
      ...
      {syncLabel && (
        <span className="ic-badge ic-badge--warn">{syncLabel}</span>
      )}
      <div style={{ flex: 1 }} />
      <button className="ic-btn" onClick={onToggleAgent}>
        {t("agent.title")}
      </button>
      <button className="ic-btn" onClick={onToggleRuns}>
        {t("canvas.run.title")}
      </button>
      <button className="ic-btn" onClick={() => setShowShortcuts(true)}>
        {t("canvas.shortcuts")}
      </button>
      <button className="ic-btn" onClick={onSaveViewport}>
        保存视口
      </button>
      <a
        className="ic-btn"
        href={`/api/v1/canvases/${canvasId}/export`}
        download
      >
        {t("assets.exportZip")}
      </a>
          ...
          >
            ...
            >
              <tbody>
                {SHORTCUTS.map(([k, v]) => (
                  <tr key={k}>
                    <td className="ic-mono" style={{ padding: "3px 0" }}>
                      {k}
                    </td>
