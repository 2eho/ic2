...
export function GenerationPanel({ t, kernel, node, onCommit, onRun }: Props) {
  ...
  const params = useMemo(() => (node.spec.params as Record<string, unknown>) ?? {}, [node.spec.params]);
  ...
  const setParam = (key: string, value: unknown) => set({ params: { ...params, [key]: value } });
  ...
  return (
    <div style={{ padding: 8, display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12, height: '100%', overflow: 'auto' }}>
      <label>
        <span className="ic-dim">{t('workbench.settings')}</span>
        <select className="ic-select" value={capability} onChange={(e) => set({ capability: e.target.value })}>
          {CAPABILITIES.map((c) => (
      ...
      <label>
        ...
        <select className="ic-select" value={String(node.spec.model ?? '')} onChange={(e) => set({ model: e.target.value })}>
          <option value="">—</option>
      ...
      {(capability === 'image.generate' || capability === 'image.edit') && (
        <>
          <label>
            ...
            <select className="ic-select" value={String(params.size ?? '1024x1024')} onChange={(e) => setParam('size', e.target.value)}>
              {['1024x1024', '1536x1024', '1024x1536', '512x512', '1792x1024'].map((s) => (
          ...
          <label>
            ...
            <select className="ic-select" value={String(params.quality ?? '')} onChange={(e) => setParam('quality', e.target.value)}>
              <option value="">—</option>
      ...
      {capability === 'video.generate' && (
        <>
          ...
          <label>
            ...
            <select className="ic-select" value={String(params.resolution ?? '720p')} onChange={(e) => setParam('resolution', e.target.value)}>
      ...
      {capability === 'audio.generate' && (
        <>
          <label>
            ...
            <input className="ic-input" value={String(params.audioVoice ?? 'alloy')} onChange={(e) => setParam('audioVoice', e.target.value)} />
          </label>
          <label>
            <span className="ic-dim">format</span>
            <select className="ic-select" value={String(params.audioFormat ?? 'mp3')} onChange={(e) => setParam('audioFormat', e.target.value)}>
      ...
      {node.result && node.result.variants && node.result.variants.length > 0 && (
