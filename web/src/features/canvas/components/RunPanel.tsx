...
export function RunPanel({ t, canvasId, tick, onClose, onUseResult }: Props) {
  ...
  return (
    <aside
      className="ic-card"
      style={{ position: 'absolute', right: 12, top: 12, width: 360, maxHeight: '70vh', overflow: 'auto', zIndex: 200, padding: 12 }}
    >
      ...
      {(q.data?.items ?? []).length === 0 && <div className="ic-empty">{t('canvas.run.empty')}</div>}
...
function RunCard({ t, run, onUseResult }: { t: TFn; run: Run; onUseResult: (nodeId: string, assetId: string) => void }) {
  const badge = run.status === 'succeeded' ? 'ic-badge--ok' : run.status === 'failed' ? 'ic-badge--danger' : run.status === 'partial' ? 'ic-badge--warn' : '';
  ...
  return (
    <div style={{ borderTop: '1px solid var(--ic-border)', paddingTop: 8, marginTop: 8 }}>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 12 }}>
        ...
        <span className="ic-dim">{t('canvas.run.cost')}: {(run.usage.costMicros / 1e6).toFixed(4)}</span>
      </div>

      {/* 计量：RunPanel 必须显示 token/张数/秒数与成本（验收要求） */}
      <div className="ic-dim" style={{ fontSize: 11, marginTop: 4 }}>
        {t('canvas.run.tokens')}: {run.usage.textTokensIn}/{run.usage.textTokensOut} · {t('canvas.run.images')}: {run.usage.images}
        {run.usage.videoMillis > 0 && ` · video: ${(run.usage.videoMillis / 1000).toFixed(1)}s`}
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
        ...
        {(run.status === 'running' || run.status === 'pending') && (
          <button className="ic-btn ic-btn--danger" style={{ fontSize: 11, padding: '3px 8px' }} onClick={() => api.cancelRun(run.id)}>
            {t('canvas.run.cancel')}
          </button>
        )}
      </div>
...
function StepRow({ t, step, onUseResult }: { t: TFn; step: RunStep; onUseResult: (nodeId: string, assetId: string) => void }) {
  const badge = step.status === 'succeeded' ? 'ic-badge--ok' : step.status === 'failed' ? 'ic-badge--danger' : step.status === 'running' || step.status === 'retrying' ? 'ic-badge--warn' : '';
  return (
    <li style={{ padding: '4px 0', fontSize: 12 }}>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
        ...
        {step.attempts.length > 1 && (
          <span className="ic-dim ic-badge--warn ic-badge">{step.attempts.length} 次尝试</span>
        )}
      </div>
      {step.error && (
        <div className="ic-error" style={{ fontSize: 11 }}>
          {t(`errors.${step.error.code}`)}
          {step.error.class && <span className="ic-dim"> · {step.error.class}</span>}
        </div>
      )}
      {step.text && <div className="ic-dim" style={{ fontSize: 11 }}>{step.text.slice(0, 120)}</div>}
