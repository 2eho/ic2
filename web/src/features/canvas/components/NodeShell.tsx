 ...
 * 端口锚点由内核计算位置，这里只渲染。
 */
export const NodeShell = memo(function NodeShell({ t, kernel, nodeId, onCommit, onRun }: Props) {
  ...
  const statusClass =
    node.state === 'failed' ? 'ic-badge--danger' : node.state === 'running' || node.state === 'pending' ? 'ic-badge--warn' : node.state === 'succeeded' ? 'ic-badge--ok' : '';
    ...
    >
      ...
      <div
        style={{
          ...
          gap: 6,
          padding: '6px 8px',
          borderBottom: node.type === 'group' ? 'none' : '1px solid var(--ic-border)',
          background: node.type === 'group' ? 'color-mix(in srgb, var(--ic-border) 30%, transparent)' : 'var(--ic-surface-2)',
          cursor: 'move',
        }}
        onDoubleClick={() => {
      ...
      >
        <span style={{ fontSize: 11, color: 'var(--ic-text-dim)' }}>{t(`canvas.node.${kindKey(node)}`)}</span>
        {editing ? (
          <input
            ref={inputRef}
            className="ic-input"
            ...
            onBlur={() => {
              kernel.dispatch({ type: 'rename', id: nodeId, title: draftTitle });
              setEditing(false);
              onCommit();
            }}
            onKeyDown={(e) => {
      ...
      <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
        <NodeContent t={t} kernel={kernel} node={node} onCommit={onCommit} onRun={onRun} />
        {node.state === 'running' && (
          <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', background: 'color-mix(in srgb, var(--ic-surface) 70%, transparent)' }}>
            <span className="ic-dim">{t('common.loading')}</span>
          </div>
        )}
        {node.error && (
          <div className="ic-error" style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 6, fontSize: 11, background: 'var(--ic-surface)' }}>
      ...
      {hover && !kernel.settings.readOnly && (
        <NodeHoverToolbar t={t} kernel={kernel} node={node} onCommit={onCommit} onRun={onRun} />
      )}
    </div>
  );
});
    ...
    <>
      {inputs.map((p, i) => (
        <span
          ...
          title={p.name}
          style={{ ...dot, left: -5, top: `${((i + 1) * 100) / (inputs.length + 1)}%` }}
        />
      ))}
      {outputs.map((p, i) => (
        <span
          ...
          data-port-kind={p.kind}
          title={p.name}
          style={{ ...dot, right: -5, top: `${((i + 1) * 100) / (outputs.length + 1)}%` }}
        />
      ))}
    </>
  );
