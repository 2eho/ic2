...
export function CanvasToolbar({ t, kernel, onCommit, onCreateNode }: Props) {
  ...
  const upload = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      ...
      const kind = asset.kind === 'image' ? 'image' : asset.kind === 'video' ? 'video' : asset.kind === 'audio' ? 'audio' : 'image';
  ...
  return (
    ...
    >
      <button className="ic-btn ic-btn--ghost" title={t('canvas.undo')} onClick={() => { if (kernel.undoOnce()) onCommit(); }}>↶</button>
      <button className="ic-btn ic-btn--ghost" title={t('canvas.redo')} onClick={() => { if (kernel.redoOnce()) onCommit(); }}>↷</button>
      <span style={{ width: 1, height: 20, background: 'var(--ic-border)' }} />

      {['prompt', 'image', 'video', 'audio', 'generation', 'group'].map((type) => (
        <button key={type} className="ic-btn ic-btn--ghost" title={NODE_SCHEMAS[type]?.title} onClick={() => onCreateNode(type)}>
          {t(`canvas.node.${type}`)}
        </button>
      ))}

      <span style={{ width: 1, height: 20, background: 'var(--ic-border)' }} />
      <button className="ic-btn ic-btn--ghost" onClick={() => fileRef.current?.click()}>
      ...
      <button
        ...
        onClick={() => {
          ...
          const next = order[(order.indexOf(kernel.settings.background as 'dots') + 1) % order.length];
          kernel.dispatch({ type: 'set-settings', settings: { background: next } });
          onCommit();
        }}
      >
        {t('canvas.background')}
      ...
      <button
        ...
        onClick={() => {
          kernel.dispatch({ type: 'set-settings', settings: { imageInfo: !kernel.settings.imageInfo } });
          onCommit();
        }}
      >
      ...
      <button
        ...
        onClick={() => {
          const group = kernel.createNode('group', {
            x: Math.min(...selection.nodes.map((id) => kernel.scene.getNode(id)?.rect.x ?? 0)) - 24,
            y: Math.min(...selection.nodes.map((id) => kernel.scene.getNode(id)?.rect.y ?? 0)) - 52,
          });
          if (group) {
            kernel.dispatch({ type: 'group', nodeIds: selection.nodes, groupId: group.id });
            onCommit();
          }
        }}
      >
      ...
      <button
        className="ic-btn ic-btn--ghost"
        disabled={selection.nodes.length === 1 && kernel.scene.getNode(selection.nodes[0])?.type !== 'group'}
        onClick={() => {
