...
describe('cropRect', () => {
  it('把越界区域裁到图像范围内', () => {
    expect(cropRect({ x: -10, y: -10, w: 500, h: 500 }, { w: 200, h: 100 })).toEqual({ x: 0, y: 0, w: 200, h: 100 });
  });
  it('宽高至少为 1', () => {
  ...
  it('NaN 输入回退到安全值', () => {
    const r = cropRect({ x: Number.NaN, y: Number.NaN, w: Number.NaN, h: Number.NaN }, { w: 100, h: 100 });
    expect(r.x).toBe(0);
    expect(r.w).toBe(1);
  });
});
...
describe('splitGrid', () => {
  ...
  it('支持自定义切线', () => {
    const cells = splitGrid({ rows: 2, cols: 1, rowCuts: [0.3] }, { w: 100, h: 100 });
    expect(cells[0].h).toBe(30);
    expect(cells[1].h).toBe(70);
  });

  it('拒绝超过单元格上限的组合', () => {
    expect(() => splitGrid({ rows: 50, cols: 50 }, { w: 100, h: 100 })).toThrow(/limit/);
  });

  it('行列被限制在合法范围', () => {
    expect(() => splitGrid({ rows: 0, cols: 0 }, { w: 100, h: 100 })).not.toThrow();
...
describe('upscaleTarget', () => {
  it('按最长边等比放大', () => {
    expect(upscaleTarget({ w: 100, h: 50 }, 200)).toEqual({ w: 200, h: 100, capped: false });
  });
...
describe('samplePixel', () => {
  ...
  it('最近邻取整点颜色', () => {
    expect(samplePixel(src, 2, 2, 0.1, 0.1, 'nearest')).toEqual([255, 0, 0, 255]);
    expect(samplePixel(src, 2, 2, 1.1, 1.1, 'nearest')).toEqual([255, 255, 255, 255]);
  });
...
describe('buildAnglePrompt', () => {
  it('角度标签与数值一致', () => {
    const p = buildAnglePrompt({ horizontal: 30, pitch: 0, distance: 1, wideAngle: false });
    expect(p).toContain('向右 30 度');
    expect(p).not.toContain('向左');
  });
  it('超出范围的角度被夹取（不会输出 200 度这种请求）', () => {
    const p = buildAnglePrompt({ horizontal: 999, pitch: -999, distance: 5, wideAngle: true });
  ...
  it('默认视角有稳定文案', () => {
    expect(buildAnglePrompt({ horizontal: 0, pitch: 0, distance: 1, wideAngle: false })).toContain('保持原视角');
  });
});
