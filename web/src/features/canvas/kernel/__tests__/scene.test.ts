...
describe("SceneGraph", () => {
  ...
  it("视口裁剪只返回相交节点", () => {
    ...
    for (let i = 0; i < 5000; i++)
      g.addNode(node("n" + i, (i % 100) * 500, Math.floor(i / 100) * 500));
  ...
  it("5000 节点下视口查询每次 < 2ms", () => {
    const g = new SceneGraph();
    for (let i = 0; i < 5000; i++)
      g.addNode(node("n" + i, (i % 100) * 400, Math.floor(i / 100) * 400));
    const t0 = performance.now();
    const rounds = 200;
    for (let i = 0; i < rounds; i++)
      g.queryRect({ x: i * 10, y: i * 5, w: 1200, h: 900 });
  ...
  it("框选为交集判定", () => {
    ...
    expect(
      g.hitRect({ x: 50, y: 50, w: 100, h: 100 }).map((n) => n.id),
    ).toEqual(["a"]);
  });
