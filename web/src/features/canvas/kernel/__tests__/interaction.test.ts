...
describe("InteractionMachine", () => {
  ...
  it("空白处拖拽进入框选并在抬起时提交", () => {
    ...
    ).toMatchObject({ type: "update-marquee" });
    expect(
      m.handle({ type: "pointerup", point: { x: 50, y: 50 } }),
    ).toMatchObject({ type: "commit-marquee" });
  ...
  it("拖拽产出相对位移", () => {
    ...
    ).toMatchObject({ type: "drag", dx: 30, dy: -10 });
  });
  ...
  it("Esc 清空选择并复位", () => {
    ...
    expect(
      m.handle({ type: "keydown", key: "Escape", modifiers: noMod }),
  ...
  it("双击节点进入文本编辑", () => {
    const m = new InteractionMachine();
    expect(
      m.handle({ type: "dblclick", point: { x: 0, y: 0 }, hitNode: "n1" }),
    ).toEqual({ type: "enter-text-edit", id: "n1" });
