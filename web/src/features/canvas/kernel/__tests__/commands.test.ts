...
describe('commandToOps', () => {
  it('移动命令展开为 delta 移动 op', () => {
    const ops = commandToOps({ type: 'move-nodes', ids: ['a', 'b'], dx: 10, dy: -5 });
    expect(ops).toHaveLength(2);
    expect(ops[0]).toEqual({ kind: 'move_node', id: 'a', x: 10, y: -5, delta: true });
  });

  it('删除命令默认级联', () => {
    expect(commandToOps({ type: 'delete-nodes', ids: ['x'] })[0]).toEqual({ kind: 'remove_node', id: 'x', cascade: true });
  });

  it('复制命令由调用方展开', () => {
    expect(commandToOps({ type: 'duplicate-nodes', ids: ['a'], offsetX: 20, offsetY: 20, newIds: {} })).toEqual([]);
  });
});
...
describe('coalesceOps', () => {
  it('同一节点多次 delta 移动合并', () => {
    ...
    expect(out.find((o) => o.kind === 'move_node' && o.id === 'a')).toEqual({ kind: 'move_node', id: 'a', x: 3, y: 4, delta: true });
  ...
  it('视口 op 只保留最后一个', () => {
    ...
    expect(vps[0]).toEqual({ kind: 'set_viewport', viewport: { x: 2, y: 2, k: 2 } });
  });

  it('零位移移动被丢弃', () => {
    expect(coalesceOps([{ kind: 'move_node', id: 'a', x: 0, y: 0, delta: true }])).toEqual([]);
  });
...
describe('applySelection', () => {
  it('非追加模式替换选择', () => {
    expect(applySelection({ nodes: ['old'], edges: [] }, { nodes: ['new'] }, false).nodes).toEqual(['new']);
  });

  it('追加模式切换选中状态', () => {
    let sel = applySelection({ nodes: ['a'], edges: [] }, { nodes: ['b'] }, true);
