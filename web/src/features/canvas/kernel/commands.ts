...
export type Command =
  ...
  | {
      type: "set-settings";
      settings: Partial<import("./types").CanvasSettings>;
    }
  | {
      type: "set-state";
      id: string;
      state: import("./types").NodeState;
      result?: import("./types").NodeResult;
...
export function commandToOps(cmd: Command): Op[] {
  switch (cmd.type) {
    ...
    case "set-spec":
      return [
        { kind: "set_spec", id: cmd.id, patch: cmd.patch, unset: cmd.unset },
      ];
    case "group":
...
export function coalesceOps(ops: Op[]): Op[] {
  ...
  for (const op of ops) {
    switch (op.kind) {
      case "move_node": {
        ...
        moveAcc.set(op.id, {
          x: prev.x + (op.delta ? op.x : 0),
          y: prev.y + (op.delta ? op.y : 0),
        });
        break;
      }
      case "resize_node":
  ...
  for (const [id, d] of moveAcc) {
    if (d.x !== 0 || d.y !== 0)
      out.push({ kind: "move_node", id, x: d.x, y: d.y, delta: true });
  }
...
export function invertOpsForUndo(
  ...
  snapshot: {
    rectOf: (id: string) => Rect | undefined;
    nodeOf: (id: string) => import("./types").RawNode | undefined;
    viewport: Viewport;
  },
): Op[] {
  const inverse: Op[] = [];
  ...
  for (let i = ops.length - 1; i >= 0; i--) {
    ...
    switch (op.kind) {
      case "move_node": {
        ...
        if (rect)
          inverse.push({ kind: "move_node", id: op.id, x: rect.x, y: rect.y });
        break;
      }
      case "resize_node": {
        const rect = snapshot.rectOf(op.id);
