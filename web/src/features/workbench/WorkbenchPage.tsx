  ...
  async function pollRun(runId: string) {
    for (let i = 0; i < 60; i++) {
      ...
      try {
        ...
        if (
          ["succeeded", "failed", "canceled", "partial"].includes(fresh.status)
        )
          return;
      } catch {
        return;
  ...
  return (
    <div style={{ display: "flex", height: "100%", minHeight: 0 }}>
          ...
          {mode === "image" ? (
            <>
              <label>
                ...
                >
                  {["1024x1024", "1536x1024", "1024x1536", "512x512"].map(
                    (s) => (
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setSeconds(
                    Math.min(30, Math.max(4, Number(e.target.value) || 4)),
                  )
                }
              />
            </label>
      ...
      <aside
        style={{
          width: 380,
          borderLeft: "1px solid var(--ic-border)",
      ...
      >
        ...
        {runs.map((r) => (
          ...
          >
            ...
            >
              <span
                className={`ic-badge ${r.status === "succeeded" ? "ic-badge--ok" : r.status === "failed" ? "ic-badge--danger" : r.status === "partial" ? "ic-badge--warn" : ""}`}
              >
                {r.status}
              </span>
            ...
            {r.error && (
              <div className="ic-error" style={{ fontSize: 12 }}>
                {t(`errors.${r.error.code}`)}
              </div>
            )}
            {r.steps.map((s) => (
              <div
