 ...
 *   - 后端地址与启动方式由外部提供（`node scripts/e2e-stack.mjs start`）。
 *   - **不用 webServer 起 vite dev**：dev server 下模块是未打包的 /src/* 路径，
 *     与生产产物行为不同；e2e 必须打「服务端同源托管的构建产物」。
 */
export default defineConfig({
  ...
  use: {
    baseURL: process.env.E2E_WEB_URL ?? 'http://127.0.0.1:18080',
