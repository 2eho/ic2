...
todo       : 8
dropped    : 3（不计入分母）
覆盖率     : 92.99%  (done / (total - dropped))
```

- 覆盖率分母排除 `dropped`（明确不做且写了理由的项）；
- **未完成项在 `10-parity-matrix.md` §14 逐条列出并写明原因**，
  避免「看起来很高」的自欺；
- 门槛对照见 `docs/design/09-roadmap.md`：当前 M5（85%）已过，M6（95%）未达。

### 覆盖率数字为什么会往下走

本轮覆盖率从 93.63% 降到 92.99%，**不是功能回退**，而是两类修正：

1. **9.5 从 `done` 改回 `wip`**：上游 34 个 Agent 工具，本仓 14 个。
   此前标 `done` 是因为按「语义等价」自评——但 MCP 客户端是按**工具名**调用的，
   名字对不上就是功能缺失。发现方式是 `docs/upstream/sync-log.md` 的上游契约面对照。
2. **9.5 补进 §14.2 的 `wip` 清单**：`done` 与 `wip` 之间不再有「看起来像完成」的灰区。

> 教训：覆盖率是**自评**指标。只有把上游契约面（工具名、op 类型、常量、i18n key）
> 拉出来逐项比对，才能发现「语义等价但契约不对等」这类假完成。这也是
> `docs/design/12 §3.3` 把探针定成「契约面而非实现细节」的原因。
...
## 上游同步链路（v0.18.0 契约面对照）中被捕获的真实缺陷

这 3 个缺陷都不在功能代码里，而在「我们以为在核对上游」这条机制自己身上：

1. **巡检判定恒报安全告警**：`report-upstream.mjs` 固定截取 CHANGELOG 开头 40 行，
   且安全判定不检查上游是否真的动了。上游 v0.17.0 的几条「Agent 令牌脱敏」修复
   永远落在窗口内 → `verdict` 每次都是 `security-fix` → 告警常态化即等于没有告警。
   修法：只取本次版本小节 + 判定前置 `upstreamMoved`；回归用例用含历史凭据条目的
   夹具钉住「上游没动就不该报安全告警」。
2. **`.gitignore` 的 `upstream/` 非锚定**，同时匹配 `docs/upstream/`：
   `sync-log.md` 与 `divergences.md` 写完**永远不会被提交**。它的隐蔽性在于
   三层假象同时成立——文件在本地存在、门禁在本地绿（门禁读文件系统）、
   `git status` 干净。修法：`/upstream/` 锚定根目录；门禁改为**问 git**
   （`git check-ignore` + `git ls-files`），因为「本地存在」不等于「会被发布」。
3. **「上游是否改 Go」无法被事实核对**：CHANGELOG v0.0.4 写「内部 Go 服务」、
   v0.4.0 写「移除后端」，只读文字会得出相反结论。修法：新增 `languages` 探针，
   夹具放一个 `main.go` 验证探针真的能认出 Go。
